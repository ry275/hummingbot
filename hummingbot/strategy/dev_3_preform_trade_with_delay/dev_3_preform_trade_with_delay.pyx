...
cdef class PerformTradeStrategy(StrategyBase):
    ...
    cdef c_process_market(self, object market_info):
        ...
        # Place order - current time is past the start time + time delay
        if self._place_orders and self._current_timestamp > self._start_timestamp + self._time_delay:
            self.logger().info("Placing orders after time delay.")
